...
describe('user creation page', function () {
    ...
    beforeEach(function () {
        ...
        return Bluebird.all([
            this.models.Person.destroy({ truncate: true })
        ]);
    });
    ...
    it('lists a user if there is one', function (done) {
        this.models.Person.create({ username: 'johndoe' }).then(function () {
            request(app).get('/').expect(/johndoe/, done);
        })
    });

    it('lists the tickets for the user if available', function (done) {
        this.models.Person.create({ username: 'johndoe' }).bind(this).then(function (user) {
